...
agg = pd.read_table("ag_score_rMATS4.txt", sep='\t|:', engine="python", header=None, skiprows=1)
agg.columns = ["Name", "Start", "End", "Score", "Freq"]
merge = agg[(agg["Score"] < 0.05) & (agg["Freq"] > 1)]

#index = len(study)
for i in study:
	sp = pd.read_table("rmats/ALL.MATS.JCEC_fix_FC_{}2.csv".format(i), header=None)
	sp.columns = ["Chr", "Start", "End", "Name", "Strand", "Diff", "abs"]
